...
class MTF:
    ...
    def adjust_for_edge_orientation(edgeImg):
        # Ermittle die Koordinaten der Kantenpunkte
        points = np.argwhere(edgeImg == 255)
        # Berechne die Steigung der Kante durch lineare Regression
        if len(points) > 0:
    ...
    @staticmethod
    def GetEdgeSpreadFunctionCrop(imgArr, filename, verbose=Verbosity.NONE):
        """
        ...
        edgeImg = cv2.Canny(np.uint8(imgArr * 255), 50, 150, apertureSize=3, L2gradient=True)

        slope, intercept = MTF.adjust_for_edge_orientation(edgeImg)

        # Prüfe die Steigung und passe die Analyse entsprechend an
        if slope is not None:
            if slope > 0:
                print("Kante verläuft von oben links nach unten rechts.")
                # Implementiere spezifische Anpassungen für diese Kantenorientierung
            else:
                print("Kante verläuft von unten links nach oben rechts.")
                # Implementiere spezifische Anpassungen für diese Kantenorientierung
        else:
            print("Keine klare Kante gefunden.")

        # Anzeige des Canny-Ergebnisses mit einer eingezeichneten Linie
        plt.figure(figsize=(20, 15))
        plt.imshow(edgeImg, cmap='gray')
        plt.title(f'Canny Edge Detection with Line - {os.path.basename(filename)}')

        # Berechne die Kante basierend auf den weißen Punkten im Canny-Ergebnis
        line = np.argwhere(edgeImg == 255)
        if len(line) > 0:  # Überprüfe, ob Linienpunkte gefunden wurden
            ...
            x = np.array([0, imgArr.shape[1]])
            y = np.polyval(finalEdgePoly, x)
            plt.plot(x, y, color='red', linewidth=3)
        else:
            print("Keine Kante gefunden")

        plt.axis('on')
        plt.show()
        # plt.pause(2)  # Anzeigedauer von 2 Sekunden
        plt.close()  # Schließen des Anzeigefensters


        # Anzeige des Canny-Kantenerkennungsbildes für eine Sekunde
        '''
        cv2.imshow('Canny Edge Detection', edgeImg)
        cv2.waitKey(1000)  # Warte 1000 Millisekunden (1 Sekunde)
        cv2.destroyAllWindows()  # Schließt das Anzeigefenster
        '''
        ...
        if angle > 0:
            ...
            finalEdgePoly[0] = -edgePoly[0]
            '''
            # Anzeige des Bildes mit der eingezeichneten Kante
            plt.figure(figsize=(20, 12))
            plt.imshow(imgArr, cmap='gray', vmin=0.0, vmax=1.0)
            x = np.array([0, imgArr.shape[1]])
            y = np.polyval(finalEdgePoly, x)
            plt.plot(x, y, color='red', linewidth=3)
            plt.title('Detected Edge with Line')
            plt.axis('on')
            plt.show()
            # plt.pause()  # Anzeigedauer von 2 Sekunden
            plt.close()  # Schließen des Anzeigefensters
            '''
    ...
    @staticmethod
    def CalculateMtf(imgArr, filename, verbose=Verbosity.NONE):
        """
        ...
        """
        imgArr = Helper.CorrectImageOrientation(imgArr)
        esf = MTF.GetEdgeSpreadFunctionCrop(imgArr, filename, Verbosity.NONE)
...
def find_mtf_threshold_frequency(mtf_x, mtf_y, threshold):
    for i in range(len(mtf_y) - 1):
        if mtf_y[i] >= threshold and mtf_y[i + 1] <= threshold:
            # Lineare Interpolation, um eine genauere Frequenz zu finden
            dx = mtf_x[i + 1] - mtf_x[i]
            dy = mtf_y[i + 1] - mtf_y[i]
            slope = dy / dx
            intercept = mtf_y[i] - slope * mtf_x[i]
            threshold_x = (threshold - intercept) / slope
            # print(f"Threshold frequency: {threshold_x}")
            return threshold_x
    return None
...
def process_image(filename):
    # Diese Funktion lädt das Bild und führt die MTF-Berechnung durch.
    imgArr = Helper.LoadImageAsArray(filename)
    # Setze verbose auf Verbosity.DETAIL, um das detaillierte Diagramm anzuzeigen.
    mtf_result = MTF.CalculateMtf(imgArr, filename, verbose=Verbosity.DETAIL)
    # Hier speichern wir zusätzlich den Dateinamen im mtf_result, um ihn später im Diagramm zu verwenden.
    mtf_result.filename = filename

    plt.pause(0.5)  # Zeigt das Diagramm für x Sekunden an.
    plt.close('all')  # Schließt alle offenen Diagrammfenster, um Ressourcen freizugeben.

    return mtf_result

def plot_combined_mtf(mtf_results):
    # MTF-Daten aller verarbeiteten Bilder in einem Diagramm plotten.
    plt.figure(figsize=(20, 15), dpi=110)
    for result in mtf_results:
        plt.plot(result.x, result.y, label=os.path.basename(result.filename))
    plt.xlabel('Spatial Frequency')
    plt.ylabel('MTF')
    plt.title('Combined MTF Graphs')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def process_directory(directory):
    ...
    for filename in sorted(os.listdir(directory)):
        if filename.startswith("normalisiert__") and (filename.endswith("__h.png") or filename.endswith("__v.png")):
            full_path = os.path.join(directory, filename)
            print(f"Processing: {full_path}")
            mtf_result = process_image(full_path)
            mtf_results.append(mtf_result)
    return mtf_results

if __name__ == "__main__":
    # directory = input("Bitte Verzeichnis eingeben: ")
    mtf_results = process_directory('/Users/toba/Documents/GitHubNeu/test_kanten/roi_temp_20240314_101331_213')
    plot_combined_mtf(mtf_results)

    for mtf_result in mtf_results:
        print(f"\nVerarbeitet: {mtf_result.filename}")
        print(f"MTF am Nyquist-Frequenz: {np.round(mtf_result.mtfAtNyquist, 2)}%")
        mtf50_frequency = find_mtf_threshold_frequency(mtf_result.x, mtf_result.y, 0.5)
        mtf20_frequency = find_mtf_threshold_frequency(mtf_result.x, mtf_result.y, 0.2)
        mtf10_frequency = find_mtf_threshold_frequency(mtf_result.x, mtf_result.y, 0.1)

        # Überprüfe, ob mtfXX_frequency None ist, bevor du np.round anwendest
        print(f"MTF50 frequency: {np.round(mtf50_frequency, 3) if mtf50_frequency is not None else 'n/a'}")
        print(f"MTF20 frequency: {np.round(mtf20_frequency, 3) if mtf20_frequency is not None else 'n/a'}")
        print(f"MTF10 frequency: {np.round(mtf10_frequency, 3) if mtf10_frequency is not None else 'n/a'}")
