...
import \
    scipy.ndimage.morphology  # pip install scipy (nicht mehr empfohlen -> deshalb "from scipy.ndimage import binary_opening")
...
import random
import \
    logging  # wurde am 20.1.24 aufgrund einer fehlermeldung bei einem bestimmten target implementiert - kann evtl. wieder weg
import tempfile
import shutil
import datetime
...
# image_path = 'testfiles/utt_060224_444dpi_big_schraeg.jpg'
# image_path = 'testfiles/utt_060224_361dpi.jpg'
### image_path = 'testfiles/color_neu_300_scharfgezeichnet.jpg'
# image_path = 'testfiles/utt_060224_361dpi_stark_geschaerft.jpg'
# image_path = 'testfiles/utt_060224_444dpi.jpg'
### image_path = 'testfiles/utt_060224_300.jpg'
# image_path = 'testfiles/utt_4k_14_170814_UTT_SG_ECI_TW9.jpg'
# image_path = 'testfiles/PAL_UTT262_240209_002_anBLHA.jpg'
# image_path = 'testfiles/color_neu.jpg'
image_path = 'testfiles/targetUTT_A300002.jpg'
# image_path = 'testfiles/targetUTT_A300002_geschaerft.jpg'
# image_path = 'testfiles/PAL_UTT262_240209_002_300dpi.tif'
...
def gather_image_info(image_path, print_info=True):
    ...
    original_pixels = None  # Initialisiere original_pixels mit None

    # Sammle EXIF-Daten, falls vorhanden
    try:
        exif_data = img_pil._getexif()  # oder img_pil.getexif() in neueren Pillow-Versionen
        exif = {
        ...
        exif = {}
        print("Keine EXIF-Daten verfügbar.")
    ...
    if original_pixels:
        ...
        change_info = "Keine Infos über ursprüngliche Pixelgröße verfügbar."
...
dpi_x, dpi_y = image11.info.get("dpi", (
None, None))  # wird gebraucht um später die ROI-Größe dynamisch an die Auflösung des Bildes anzupassen
...
template = cv2.imread('testfiles/edge_grau.jpg',
                      0)  # Template-Muster zur erkennung der bereich für slanted edge in Graustufen
...
for pt in zip(*locations[::-1]):  # Switch columns and rows
    rectangles.append(
        [int(pt[0]), int(pt[1]), int(pt[0] + template_adjusted.shape[1]), int(pt[1] + template_adjusted.shape[0])])
...
lines_array = np.array(
    filtered_lines)  # filtered_lines von Tulpel in ein NumPy-Array konvertieren (für funktion filter_isolated_lines)
    ...
    for row in range(3):
        for col in range(3):
            ...
            for line in cell_lines:
                ...
                distances[line_tuple] = calculate_distance_to_edges(line_tuple, cell_x_min, cell_y_min, cell_x_max,
                                                                    cell_y_max)
            ...
            sorted_keys = sorted(distances.keys(), key=lambda l: distances[
                l])  # Beispiel, wie man basierend auf den Distanzen sortieren könnte
...
def draw_connected_lines_with_ids_by_cell(image, lines, color='yellow', thickness=2, text_size=12):
    ...
    def visualize_lines_and_squares(image, json_path, json_path_square, color='yellow', thickness=1, text_size=9):
        ...
        # pp.show(block=False)
        # pp.show()
        # pp.pause(10)  # Anzeige für 2 Sekunde pausieren
        pp.close()  # Fenster schließen
...
def show_roi_images(image_path, json_path_square):
    try:
        ...
        for roi_coord, roi_image in zip(roi_coordinates, roi_images):
            ...
            cv2.putText(combined_image, roi_id, (col_index + 10, row_index + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
...
def process_images(temp_dir):
    try:
        ...
        normalisierte_files = [file for file in os.listdir(temp_dir) if
                               file.startswith("normalisiert__") and (
                                           file.endswith("__v.png") or file.endswith("__h.png"))]
        ...
        if file.startswith("normalisiert__") and (file.endswith("__v.png") or file.endswith("__h.png")):
            ...
            try:
                ...
                if edge_straight is not None:
                    ...
                    results.append((esf, lsf, lsfs, mtf, mtfs, edge, edge_angle, x_mtf, mtf50, mtf20, mtf10, lsfmax,
                                    lsfmin, lsfunc, individual_id))
...
def plot_mtf_and_esf(temp_dir, edge, individual_id, esf, lsf, lsfs, mtf, mtfs, edge_angle, x_mtf, mtf50, mtf20, mtf10,
                     lsfmax, lsfmin, lsfunc):
    ...
    pp.ylabel('Intensity')
    pp.grid(True, linestyle=":", linewidth=0.5)
    pp.legend()

    # Plot LSF
    pp.subplot(1, 4, 2)
    pp.plot(lsf, label='LSF', color='lightblue', linewidth=0.5)
    ...
    # Plot MTF
    pp.subplot(1, 4, 3)
    pp.plot(x_mtf, mtf, label='Unfiltered', color='lightblue', linewidth=0.5)
    pp.plot(x_mtf, mtfs, label='Filtered')
    pp.title(f'MTF - {individual_id}')
    pp.axhline(y=0.5, color='r', linestyle='--', label=f'MTF50 {np.round(mtf50, 3)}')
    pp.axhline(y=0.2, color='g', linestyle='--', label=f'MTF20 {np.round(mtf20, 3)}')
    pp.axhline(y=0.1, color='b', linestyle='--', label=f'MTF10 {np.round(mtf10, 3)}')
    ...
    pp.figtext(0.01, 0.99, f'Aktualisiert am: {time_str} - {os.path.basename(image_path)}', ha="left", va="top",
...
if results:
    for result in results:
        ...
        plot_mtf_and_esf(temp_dir, edge, individual_id, esf, lsf, lsfs, mtf, mtfs, edge_angle, x_mtf, mtf50, mtf20,
                         mtf10, lsfmax, lsfmin, lsfunc)
...
def plot_all_mtfs(results, **kwargs):
    ...
    p1 = f"MTF50   :  {np.round(np.mean(mtf50_values), 3)} | Min: {np.round(np.min(mtf50_values), 3)}  Max: {np.round(np.max(mtf50_values), 3)}  Std: {np.round(np.std(mtf50_values), 3)}"
    p2 = f"MTF20   :  {np.round(np.mean(mtf20_values), 3)} | Min: {np.round(np.min(mtf20_values), 3)}  Max: {np.round(np.max(mtf20_values), 3)}  Std: {np.round(np.std(mtf20_values), 3)}"
    p3 = f"MTF10   :  {np.round(np.mean(mtf10_values), 3)} | Min: {np.round(np.min(mtf10_values), 3)}  Max: {np.round(np.max(mtf10_values), 3)}  Std: {np.round(np.std(mtf10_values), 3)}"
    p4 = f"LSFUNC: {np.round(np.mean(lsfunc_values), 4)} | Min: {np.round(np.min(lsfunc_values), 3)}  Max: {np.round(np.max(lsfunc_values), 3)}  Std: {np.round(np.std(lsfunc_values), 3)}"
    ...
    for result in sorted_results:
        ...
        pp.plot(x_mtf, mtfs,
                label=f'ID: {individual_id} /mtf50: {mtf50_rounded} /mtf20: {mtf20_rounded} /mtf10: {mtf10_rounded}')
        pp.plot(x_mtf, mtf, **kwargs, color='lightgray', linewidth=0.5)
    ...
    # Platzieren von Text an einer bestimmten Position
    pp.text(-0.26, 1.14, "Durchschnittswerte und Standardabweichung", fontsize=10, ha='left', fontweight='bold')
    pp.text(-0.26, 1.12, p1, fontsize=10, ha='left')
    pp.text(-0.26, 1.10, p2, fontsize=10, ha='left')
    pp.text(-0.26, 1.08, p3, fontsize=10, ha='left')
    pp.text(-0.26, 1.06, p4, fontsize=10, ha='left')
    # ---------sichern der ansicht ----------
...
# ist nachfolgend eine funktion mit einer kombination aus cv2 und matplotlib implementiert - !damit mittels matplotlib auf cv2 visualisierung gezoomt werden kann!

def visualize_with_cv2_text(image_path, json_path_square, results, image_info, square_size=80):
    # Aktuelles Datum und Uhrzeit erhalten
    current_time = datetime.datetime.now()
    # Formatieren des Datums und der Uhrzeit als String
    time_str = current_time.strftime('%Y-%m-%d %H:%M:%S')
    # Bild laden
    image = cv2.imread(image_path)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Konvertiere BGR zu RGB für Matplotlib

    # JSON-Datei laden
    with open(json_path_square, 'r') as file:
        squares_data = json.load(file)['squares']

    # Zeichne Quadrate und Text mit OpenCV
    for square_id, square in squares_data.items():
        cleaned_square_id = square_id.split('__')[0]
        square_coords = np.array(square)
        mid_coords = np.mean(square_coords, axis=0)

        # Suche die MTF-Werte für die aktuelle ID
        for _, _, _, _, _, _, _, _, mtf50, _, mtf10, _, _, _, individual_id in results:
            if individual_id == cleaned_square_id:
                # Bestimme die Füllfarbe basierend auf MTF10-Werten
                if mtf10 <= 0.65:
                    fill_color = (0, 0, 255)  # Rot
                elif 0.65 < mtf10 <= 0.75:
                    fill_color = (0, 255, 255)  # Gelb
                else:
                    fill_color = (0, 255, 0)  # Grün

                # Zeichne ein gefülltes Quadrat
                cv2.rectangle(image, (int(mid_coords[0] - square_size / 2), int(mid_coords[1] - square_size / 2)),
                              (int(mid_coords[0] + square_size / 2), int(mid_coords[1] + square_size / 2)), fill_color,
                              -1)

                # Füge Text hinzu
                cv2.putText(image, f'{individual_id}', (int(mid_coords[0] - 20), int(mid_coords[1] - 31)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 5, cv2.LINE_AA)
                cv2.putText(image, f'{individual_id}', (int(mid_coords[0] - 20), int(mid_coords[1] - 31)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(image, f'MTF50: {mtf50:.2f}', (int(mid_coords[0] - 20), int(mid_coords[1])),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 5, cv2.LINE_AA)
                cv2.putText(image, f'MTF50: {mtf50:.2f}', (int(mid_coords[0] - 20), int(mid_coords[1])),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(image, f'MTF10: {mtf10:.2f}', (int(mid_coords[0] - 20), int(mid_coords[1] + 31)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 5, cv2.LINE_AA)
                cv2.putText(image, f'MTF10: {mtf10:.2f}', (int(mid_coords[0] - 20), int(mid_coords[1] + 31)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2, cv2.LINE_AA)

    # Konvertiere das Bild zurück zu RGB für Matplotlib
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    pp.imshow(image_rgb)
    # Anzeige von Datum und Uhrzeit im Plot, oben links
    pp.figtext(0.01, 0.98, f'Aktualisiert am: {time_str} - {os.path.basename(image_path)}', ha="left", va="top",
               fontsize=10, fontweight='bold', bbox={"facecolor": "orange", "alpha": 0.5, "pad": 5})
    pp.figtext(0.01, 0.94,
               f'Durchschnittswerte:\nMTF50 :  {np.round(np.mean(mtf50_values), 3)} | Min: {np.round(np.min(mtf50_values), 3)}  Max: {np.round(np.max(mtf50_values), 3)}  Std: {np.round(np.std(mtf50_values), 3)}\nMTF20 :  {np.round(np.mean(mtf20_values), 3)} | Min: {np.round(np.min(mtf20_values), 3)}  Max: {np.round(np.max(mtf20_values), 3)}  Std: {np.round(np.std(mtf20_values), 3)}\nMTF10 :  {np.round(np.mean(mtf10_values), 3)} | Min: {np.round(np.min(mtf10_values), 3)}  Max: {np.round(np.max(mtf10_values), 3)}  Std: {np.round(np.std(mtf10_values), 3)}\nLSFUNC: {np.round(np.mean(lsfunc_values), 4)} | Min: {np.round(np.min(lsfunc_values), 3)}  Max: {np.round(np.max(lsfunc_values), 3)}  Std: {np.round(np.std(lsfunc_values), 3)}',
               ha="left", va="top", fontsize=10, bbox={"facecolor": "lightblue", "alpha": 0.8, "pad": 5})

    # -------- EXIF-Daten für textausgabe formatieren
    exif_data = image_info.get('EXIF-Daten', {})
    if not exif_data:
        exif_text = "EXIF-Daten: Nicht verfügbar\n"
    else:
        exif_text = "EXIF-Daten:\n"
        for key, value in exif_data.items():
            exif_text += f"{key}: {value}\n"  # Jedes Paar in einer neuen Zeile
    # --------ENDE EXIF-Daten für textausgabe formatieren

    # --------Image Info und EXIF in einem Kasten darstellen
    info_text = (
        f"Dateiname: {image_info['Dateiname']}\n"
        f"Größe: {image_info['Größe in Pixel']}\n"
        f"Kanäle: {image_info['Farbkanäle']}\n"
        f"DPI: {image_info.get('DPI', 'Nicht verfügbar')}\n"
        f"Veränderung: {image_info.get('Veränderung')}\n"
        + exif_text  # EXIF-Daten hinzufügen
    )

    pp.figtext(0.01, 0.83, info_text, ha="left", va="top", fontsize=9, bbox={"facecolor": "lightgreen", "alpha": 0.8, "pad": 5})
    # --------ENDE Image Info und EXIF in einem Kasten darstellen

    pp.axis('on')
    # ---------sichern der ansicht ----------
    output_filename = os.path.join(temp_dir, f"all_mtf_status.png")
...
image_info = gather_image_info(image_path, print_info=False) # print_info=False verhindert das ausgeben der bildinformationen im terminal

# folgende visualisierung nutzt nur matplotlib ohne cv2, hier gibt es das text-zoom-problem
# visualize_lines_and_squares_with_colored_squares(image_path, json_path_square, results)

# folgende visualisierung nutzt cv2 und zeigt dann das bild in matplotlib an, hier gibt es das text-zoom-problem nicht
visualize_with_cv2_text(image_path, json_path_square, results, image_info)
...
def calculate_averages(results):
    ...
    print(
        f"MTF50 :  {np.round(np.mean(mtf50_values), 3)} | Min: {np.round(np.min(mtf50_values), 3)}  Max: {np.round(np.max(mtf50_values), 3)}  Std: {np.round(np.std(mtf50_values), 3)}")
    print(
        f"MTF20 :  {np.round(np.mean(mtf20_values), 3)} | Min: {np.round(np.min(mtf20_values), 3)}  Max: {np.round(np.max(mtf20_values), 3)}  Std: {np.round(np.std(mtf20_values), 3)}")
    print(
        f"MTF10 :  {np.round(np.mean(mtf10_values), 3)} | Min: {np.round(np.min(mtf10_values), 3)}  Max: {np.round(np.max(mtf10_values), 3)}  Std: {np.round(np.std(mtf10_values), 3)}")
    print(
        f"LSFUNC: {np.round(np.mean(lsfunc_values), 4)} | Min: {np.round(np.min(lsfunc_values), 3)}  Max: {np.round(np.max(lsfunc_values), 3)}  Std: {np.round(np.std(lsfunc_values), 3)}")
    print('---------------------------------------------------')
    print(
        'BTW: Die Standardabweichung ist ein Maß für die Streuung oder Variabilität von Datenpunkten in einer Datensammlung im Vergleich\nzum Durchschnitt (Mittelwert) dieser Daten. Sie gibt an, wie weit die einzelnen Datenpunkte im Durchschnitt vom Mittelwert entfernt liegen.\nEine geringe Standardabweichung bedeutet, dass die Datenpunkte im Allgemeinen nah am Mittelwert liegen, während eine hohe Standardabweichung\ndarauf hinweist, dass die Datenpunkte über einen breiteren Bereich verteilt sind.')
