...
# folgende visualisierung nutzt nur matplotlib ohne cv2, hier gibt es das text-zoom-problem, deswegen
# ist nachfolgend noch eine alternative funktion mit einer kombination aus cv2 und matplotlib implementiert
'''
def visualize_lines_and_squares_with_colored_squares(image_path, json_path_square, results, square_size=80, thickness=1):
    # Bild laden
    ...
    # Figur für das Zeichnen vorbereiten
    fig, ax = pp.subplots(figsize=(18, 10))
    ax.imshow(image_rgb)
    ax.axis('off')
    ax.set_title('Visualisierung von MTF-Werten mit farbigen Quadraten')

    # Überprüfe, ob MTF-Werte für jede ID gefunden werden
    mtf_found = False

    # Zeichne Quadrate mit farbiger Füllung basierend auf MTF-Werten
    for square_id, square in squares_data.items():
        ...
        for _, _, _, _, _, _, _, _, mtf50, _, mtf10, _, _, _, individual_id in results:
            if individual_id == cleaned_square_id:
                ...
                # Zeichne ein gefülltes Quadrat an den Mittelpunkt der jeweiligen ROI
                ax.add_patch(Rectangle((mid_coords[0] - square_size / 2, mid_coords[1] - square_size / 2), square_size,
                                       square_size, color=fill_color, alpha=0.8))

                # Zeige die ID und MTF-Werte neben dem Quadrat
                ax.text(mid_coords[0] + square_size / 2 + 10, mid_coords[1],
                        f'ID: {individual_id}\nMTF50: {mtf50:.2f}\nMTF10: {mtf10:.2f}', color='cyan', fontsize=6,
                        ha='left', va='center')
    ...
    if not mtf_found:
        print("Keine MTF-Werte für die gegebenen IDs gefunden. Überprüfe die Übereinstimmung der IDs.")

    pp.show()
'''
def visualize_with_cv2_text(image_path, json_path_square, results, square_size=80):
    # Aktuelles Datum und Uhrzeit erhalten
    current_time = datetime.datetime.now()
    # Formatieren des Datums und der Uhrzeit als String
    time_str = current_time.strftime('%Y-%m-%d %H:%M:%S')
    ...
    # Setze den Dateinamen für das zu speichernde Bild fest
    output_filename = os.path.join(temp_dir, "all_mtf_status.jpg")
        ...
        for _, _, _, _, _, _, _, _, mtf50, _, mtf10, _, _, _, individual_id in results:
            if individual_id == cleaned_square_id:
                ...
                cv2.rectangle(image, (int(mid_coords[0] - square_size / 2), int(mid_coords[1] - square_size / 2)),
                              (int(mid_coords[0] + square_size / 2), int(mid_coords[1] + square_size / 2)), fill_color, -1)
...
visualize_with_cv2_text(image_path, json_path_square, results)
